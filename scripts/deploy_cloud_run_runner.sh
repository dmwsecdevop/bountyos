...
exec gcloud run deploy "$SERVICE_NAME" \
  ...
  --max-instances=1 \
  --concurrency=80 \
  --set-env-vars "BOUNTYOS_AI_PROVIDER=vertex,GOOGLE_CLOUD_PROJECT=${PROJECT_ID},GOOGLE_CLOUD_LOCATION=global,GOOGLE_GENAI_USE_VERTEXAI=true,BOUNTYOS_LIGHT_MODEL=gemini-2.5-flash-lite,BOUNTYOS_RECON_MODEL=gemini-2.5-flash,BOUNTYOS_MAIN_MODEL=gemini-2.5-pro,BOUNTYOS_AGGRESSIVE_MODEL=gemini-2.5-pro,BOUNTYOS_EXPLOIT_MODEL=gemini-2.5-pro,BOUNTYOS_EXECUTION_MODE=hybrid" \
  "$@"
