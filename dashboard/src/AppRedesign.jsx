...
import IntegrationConfig from './IntegrationConfig.jsx';
...
function listText(items) {
  return asList(items).map(item => typeof item === 'string' ? item : item?.next_safe_action || item?.label || item?.name || item?.tool || JSON.stringify(item)).filter(Boolean);
}
...
function cleanSummary(text, raw) {
  ...
  if (guidance.confidence || guidance.impact) lines.push([guidance.confidence ? `Confidence: ${guidance.confidence}.` : '', guidance.impact ? `Impact: ${guidance.impact}.` : ''].filter(Boolean).join(' '));
...
function normalizeResponse(data, fallbackSummary = 'Done.') {
  ...
  const parsed = parseJsonText(rawSummary);
  const guidance = parsed?.operational_guidance || parsed?.guidance || parsed;
  return {
    ...emptyMessage,
    summary: cleanSummary(rawSummary, data),
    actions: listText(act.next_actions || data?.next_actions || act.actions || data?.actions || guidance?.next_safe_actions || guidance?.next_actions).slice(0, 6),
    logs: asList(act.logs || data?.logs || act.output || data?.output).map(item => typeof item === 'string' ? item : JSON.stringify(item, null, 2)),
    evidence: asList(act.evidence || data?.evidence || act.findings || data?.findings),
    planner: data?.think || act.planner || data?.planner || null,
    raw: data,
    scan_id: act.scan_id || data?.scan_id || data?.reason?.scan_id || null,
    target_id: act.target_id || data?.target_id || data?.reason?.target_id || null,
    events: asList(act.events || data?.events || data?.scan_events),
  };
}

function Badge({children, tone = 'neutral'}) { return <span className={`badge ${tone}`}>{children}</span>; }
function StatusCard({title, children, action}) { return <section className="status-card"><div className="status-title"><h3>{title}</h3>{action}</div>{children}</section>; }

function Expanders({message}) {
  const groups = [['Execution logs', message.logs], ['Raw JSON', message.raw], ['Planner details', message.planner], ['Scan events', message.events], ['Evidence', message.evidence]];
  return <div className="expanders">{groups.map(([label, value]) => {
    const hasValue = Array.isArray(value) ? value.length > 0 : Boolean(value);
    if (!hasValue) return null;
    return <details key={label}><summary>{label}</summary><pre>{typeof value === 'string' ? value : JSON.stringify(value, null, 2)}</pre></details>;
  })}</div>;
}

function ChatMessage({message}) {
  return <article className={`message ${message.role}`}>
    <div className="message-meta"><span>{message.role === 'user' ? 'You' : message.role === 'system' ? 'System' : 'Hunter Brain'}</span>{message.model && <Badge>{message.model}</Badge>}</div>
...
function useOpsData() {
  ...
  const refresh = async () => {
    const [targets, scans, findings, live, runners, models] = await Promise.all([
      api('/targets/').catch(() => []), api('/scans/').catch(() => []), api('/findings/').catch(() => []), api('/live/snapshot').catch(() => null), api('/runners/capabilities').catch(() => null), api('/ai/models').catch(() => null),
    ]);
    setData({targets, scans, findings, live, runners, models});
  };
  useEffect(() => { refresh(); const id = setInterval(refresh, 4500); return () => clearInterval(id); }, []);
  return {...data, refresh};
}

function Sidebar({data, selectedTargetId, selectedScanId, onSelectTarget, onSelectScan}) {
  ...
  return <aside className="right-rail">
    <StatusCard title="System" action={<button className="pill-button" onClick={data.refresh}>Refresh</button>}>
      <div className="signal-row"><span className={`signal ${runner ? 'on' : 'off'}`}/><div><b>{runner ? 'Runner online' : 'Runner offline'}</b><small>{runner?.name || 'Start local runner'}</small></div></div>
    ...
    <StatusCard title="Active Target"><select value={selectedTargetId} onChange={e => onSelectTarget(e.target.value)}><option value="">No target selected</option>{data.targets.map(t => <option key={t.id} value={t.id}>{t.name || t.domain}</option>)}</select><div className="compact-copy"><b>{selectedTarget?.domain || 'No target yet'}</b><span>{truncate(selectedTarget?.scope || 'Create or select a target.', 140)}</span></div></StatusCard>
    <StatusCard title="Scan Status"><select value={selectedScanId} onChange={e => onSelectScan(e.target.value)}><option value="">No scan selected</option>{data.scans.map(s => <option key={s.id} value={s.id}>{shortId(s.id)} · {s.status}</option>)}</select><div className="compact-copy"><b>{selectedScan ? `${selectedScan.mode || 'scan'} · ${selectedScan.status}` : 'Idle'}</b><span>{selectedScan ? `Phase ${selectedScan.phase || 'queued'} · ${shortId(selectedScan.id)}` : 'Start from Hunter Brain.'}</span></div></StatusCard>
  </aside>;
}
...
function LogsDrawer({open, live}) {
  ...
  return <section className={`logs-drawer ${open ? 'open' : ''}`}><div className="drawer-handle"><span>Live logs</span><Badge>{events.length}</Badge></div><div className="log-list">{events.map((event, i) => <div className="log-row" key={i}><span>{event.kind}</span><p>{truncate(event.message, 180)}</p></div>)}{!events.length && <p className="muted">No live logs yet.</p>}</div></section>;
}

function FindingsPage({data}) { return <section className="page-card"><h2>Findings</h2><p className="muted">Parsed findings and evidence. Chat stays clean.</p><div className="finding-grid">{data.findings.map(f => <div className="finding-card big" key={f.id}><Badge tone={String(f.severity).toLowerCase()}>{f.severity || 'info'}</Badge><b>{f.title}</b><small>{f.tool || 'unknown tool'} · {f.url || 'no URL'}</small><p>{truncate(f.description || f.evidence || 'No detail yet.', 260)}</p></div>)}{!data.findings.length && <p className="muted">No parsed findings yet.</p>}</div></section>; }

function ToolsPage({data}) { const runner = data.runners?.online?.[0]; const tools = Object.entries(runner?.tools || {}); return <section className="page-card"><h2>Tools</h2><p className="muted">Runner inventory and versions.</p><div className="tool-grid">{tools.map(([name, meta]) => <div className="tool-card" key={name}><b>{name}</b><small>{meta.path || meta.binary}</small><p>{truncate(meta.version || 'version unknown', 110)}</p></div>)}{!tools.length && <p className="muted">Runner is offline or no tools reported.</p>}</div></section>; }

function SettingsPage() { return <section className="page-card"><h2>Settings</h2><p className="muted">Personal-use BountyOS settings. Keep active validation approval-gated and stay inside authorized program scope.</p><div className="settings-list"><div><b>Safety</b><span>Active checks require approval.</span></div><div><b>Logs</b><span>Raw JSON and stdout stay collapsed until opened.</span></div><div><b>Layout</b><span>One clean command page, separate pages for integrations/tools/findings.</span></div></div></section>; }
...
export default function AppRedesign() {
  ...
  const runAction = async (label, fn) => { setBusy(true); try { await fn(); } catch (e) { push({...emptyMessage, role: 'assistant', summary: `${label} failed: ${e.message}`, raw: e.payload || {error: e.message, status: e.status}}); } finally { setBusy(false); data.refresh(); } };
  ...
  return <div className="ops-shell"><main className="chat-panel"><header className="topbar"><div><p className="eyebrow">BountyOS v6.5</p><h1>{pages[view]}</h1></div><nav className="topbar-badges">{Object.entries(pages).map(([key, label]) => <button key={key} className={view === key ? 'nav-button active' : 'nav-button'} onClick={() => setView(key)}>{label}</button>)}<Badge tone={(data.runners?.online || []).length ? 'green' : 'red'}>{(data.runners?.online || []).length ? 'runner online' : 'runner offline'}</Badge><Badge tone="cyan">{data.models?.chat_model || 'gemini'}</Badge></nav></header>
    ...
    {view === 'findings' && <FindingsPage data={data}/>} {view === 'integrations' && <IntegrationConfig/>} {view === 'tools' && <ToolsPage data={data}/>} {view === 'settings' && <SettingsPage/>}</main><Sidebar data={data} selectedTargetId={selectedTargetId} selectedScanId={selectedScanId} onSelectTarget={setSelectedTargetId} onSelectScan={setSelectedScanId}/></div>;
}
